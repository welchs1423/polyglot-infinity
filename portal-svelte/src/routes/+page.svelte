		...
		{#if erlangData}
			...
			{:else}
				...
				<div class="julia-card erlang-card">
					...
					<span class="value"
						>{(
							(erlangData.riskBefore?.risk_score ?? 0) * 100
						).toFixed(2)}% {erlangData.riskBefore?.grade}</span
					>
				</div>
				<div
					class="julia-card erlang-card"
				...
				>
					...
					<span class="value" style="color:#f59e0b"
						>{(
							(erlangData.riskAfter?.risk_score ?? 0) * 100
						).toFixed(2)}% {erlangData.riskAfter?.grade}</span
					>
				</div>
				<div class="julia-card erlang-card">
