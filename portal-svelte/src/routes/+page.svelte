	...
	async function runErlang() {
		...
		try {
			const [status, swap, riskV1] = await Promise.all([
				fetch('http://localhost:4003/api/erlang/status').then((x) => x.json()),
				fetch('http://localhost:4003/api/erlang/hotswap').then((x) => x.json()),
				fetch('http://localhost:4003/api/erlang/risk?debt=0.65&vol=0.28').then((x) => x.json()),
			]);
			// risk after swap uses the new logic version now active
			const riskV2 = await fetch(
				'http://localhost:4003/api/erlang/risk?debt=0.65&vol=0.28',
			).then((x) => x.json());
			erlangData = { ...status, swap, riskBefore: riskV1, riskAfter: riskV2 };
		} catch {
...
<main class="container">
						...
						<div class="julia-grid">
							<div class="julia-card haskell-card">
								<span class="jlabel">Final Price</span><span
									class="jval"
									>{haskellData.stream_final?.toFixed(
										2,
									)}</span
								>
							</div>
							<div class="julia-card haskell-card">
								<span class="jlabel">Sharpe (stream)</span><span
									class="jval"
									>{haskellData.stream_sharpe?.toFixed(
										3,
									)}</span
								>
							</div>
							...
							<div class="julia-card haskell-card">
								<span class="jlabel">EWMA Vol (last)</span><span
									class="jval"
									>{(
										(haskellData.stream_ewma_vol?.at(-1) ??
											0) * 100
									).toFixed(3)}%</span
								>
							</div>
	...
	<section class="panel erlang-panel">
		...
		<p class="subtitle">code:load_file/1 — 서버 무중단 로직 교체 · 진행 중 연결은 구버전으로 완료 (BEAM 2-version protocol)</p>
		<button class="erlang-btn" onclick={runErlang} disabled={erlangLoading}>
			{erlangLoading ? '핫 스왑 중...' : '핫 스왑 실행'}
		</button>
		{#if erlangData}
			{#if erlangData.error}
				<div class="error-box"><p style="color:#f87171">{erlangData.error}</p></div>
			{:else}
				<div class="julia-card erlang-card" style="border-color:#ef4444">
					<span class="label">다운타임</span>
					<span class="value" style="color:#34d399">{erlangData.swap?.downtime_ms}ms</span>
				</div>
				...
				<div class="julia-card erlang-card">
					<span class="label">{erlangData.swap?.old_logic} → {erlangData.swap?.new_logic}</span>
					<span class="value">교체 완료</span>
				</div>
				<div class="julia-card erlang-card">
					<span class="label">리스크 (스왑 전)</span>
					<span class="value">{((erlangData.riskBefore?.risk_score ?? 0) * 100).toFixed(2)}% {erlangData.riskBefore?.grade}</span>
				</div>
				<div class="julia-card erlang-card" style="border-color:#f59e0b">
					<span class="label">리스크 (스왑 후)</span>
					<span class="value" style="color:#f59e0b">{((erlangData.riskAfter?.risk_score ?? 0) * 100).toFixed(2)}% {erlangData.riskAfter?.grade}</span>
				</div>
				<div class="julia-card erlang-card">
		...
		{:else}
			<div class="empty-box">
				<p>버튼을 눌러 리스크 로직을 핫 스왑하세요. (Erlang 서버 :4003 필요)</p>
			</div>
		{/if}
	</section>
